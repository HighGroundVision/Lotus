<template>
  <div style="border: 2px solid rgba(255, 255, 255, 0.2); border-radius: 10px; padding: 30px; max-width: 800px; width: 100%; margin: auto; margin-top: 10px; margin-bottom: 50px; display: flex; align-items: center">
    <div style="padding: 5px; width: 100%">
      <div style="font-size: 30px; margin-bottom: 5px">Captains</div>
      <div style="font-size: 14px; color: rgba(255, 255, 255, 0.5)">You must select the same side that you are in the Dota lobby.</div>
      <div style="display: flex; text-align: center; margin-top: 10px">
        <div style="width: 100%">
          <div class="radiant-header">
            <h1>RADIANT</h1>
          </div>
          <template v-for="item in players.slice(1, 2)" :key="item.id">
        ...
        <div style="width: 100%">
          ...
          <template v-for="item in players.slice(2)" :key="item.id">
            <div v-if="item.name" class="jnbria">
              <span>{{ item.name }}</span>
            </div>
            <div v-else @click="claim(item.id)" class="jnbrig">
