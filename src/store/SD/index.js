...
let lobbyClient = new LobbyClient({
...
const SingleDraftGameStore = {
  ...
  actions: {
    ...
    startClient({ commit, state }) {
      if (state.isConnected) gameClient.stop()
      gameClient.updateMatchID(state.match.matchID)
      gameClient.updatePlayerID(state.slot?.playerID)
      gameClient.updateCredentials(state.slot?.playerCredentials)
      gameClient.subscribe((gs) => {
        if (gs) {
          commit('updateGameState', gs)
          commit('setSlots', gameClient.matchData)
        }
      })
      gameClient.start()
    },
    ready() {
      gameClient.moves.Readyup()
    },
    pick(_, key) {
      gameClient.moves.PickHero(key)
    },
    pickExtra(_, { index, id }) {
      let hero = db.slice(0).find((i) => i.id == id)
      gameClient.moves.PickExtra(index, hero)
    },
    voteExtra(_, index) {
      gameClient.moves.VoteExtra(index)
    },
  },
  getters: {
    matchID(state) {
